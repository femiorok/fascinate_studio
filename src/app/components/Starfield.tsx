...
if (this.bright) {
  ...
  ctx.lineWidth = this.z * 1.5; // Increase the line width for enhanced visibility
      ...
      const gradient = ctx.createLinearGradient(this.xPrev, this.yPrev, this.x, this.y);
...
const Starfield = () => {
  ...
  useEffect(() => {
    ...
    function setup() {
      ...
      const stars = Array.from({ length: COUNT }, () => new Star(Math.random() * width - width / 2, Math.random() * height - height / 2));
...
return (
    <div ref={containerRef} className='absolute inset-0 h-full' style={{background: 'linear-gradient(0deg, black 120%, transparent)'}}>
